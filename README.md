...
Software desktop in Java sviluppato durante il percorso PCTO presso Galileo Italia SRL. Dato in input la foto di un tessuto, il programma usa un modello linguistico multimodale (via Ollama) per individuare e descrivere i difetti presenti, producendo un report con localizzazione, gravità e note tecniche.
...
- **Cronologia e cache**: ogni report viene salvato in SQLite; i risultati vengono cachati per immagine + tipo di tessuto.
...
L'interfaccia grafica desktop è in Java Swing con FlatLaf Dark Theme: controlli di upload immagine, selettore tipologia tessuto, progress bar indeterminata durante l'inferenza AI e pannello di visualizzazione diagnostica.
...
- Risposta: campo `message.content` con il report diagnostico strutturato.

## Decisioni di engineering

- **Integrazione Ollama via HTTP diretto**: nessun client dedicato (es. ollama4j); si usa `HttpClient` nativo di Java 11+ più Jackson. Meno dipendenze e controllo sul payload.
...
- **Preprocessing aggressivo**: downscale a 512 px e compressione JPEG riducono i token visivi inviati al modello, mantenendo qualità sufficiente per i difetti.
- **Parametri a bassa temperatura**: scelti per output coerenti, adatti a un contesto industriale di QC.
- **Ant/NetBeans come build**: coerente con l'ambiente scolastico/aziendale di sviluppo.

> Nota sui modelli: il codice usa `gemma3:1b`, mentre README/CLI citano `gemma3:4b` e la GUI mostra "Powered by Qwen 2.5 VL". La storia del repo mostra una migrazione reale tra modelli; adeguare il nome nel codice e nell'UI alla versione effettivamente usata.

## Limiti e prossimi passi

- Il nome del modello è incoerente tra codice, CLI e GUI (vedi nota sopra).
- `dist/lib/` contiene solo i jar Jackson: FlatLaf e SQLite JDBC non sono copiati nella distribuzione, quindi il jar richiede lib aggiuntive.
